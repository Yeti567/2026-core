...
CREATE TABLE document_control_sequences (
...
CREATE INDEX idx_doc_seq_company ON document_control_sequences(company_id);

-- Documents - Primary indexes
CREATE UNIQUE INDEX idx_doc_control_number ON documents(company_id, control_number);
CREATE INDEX idx_doc_type ON documents(company_id, document_type_code);
CREATE INDEX idx_doc_status ON documents(company_id, status);
CREATE INDEX idx_doc_review_date ON documents(company_id, next_review_date) WHERE status = 'active';
CREATE INDEX idx_doc_effective_date ON documents(company_id, effective_date);
CREATE INDEX idx_doc_expiry_date ON documents(company_id, expiry_date) WHERE expiry_date IS NOT NULL;
CREATE INDEX idx_doc_created_by ON documents(created_by);
CREATE INDEX idx_doc_updated_at ON documents(updated_at DESC);

-- Full-text search
CREATE INDEX idx_doc_search ON documents USING gin(search_vector);
CREATE INDEX idx_doc_text_search ON documents USING gin(to_tsvector('english', COALESCE(title, '') || ' ' || COALESCE(description, '') || ' ' || COALESCE(extracted_text, '')));

-- Array indexes (GIN for containment queries)
CREATE INDEX idx_doc_cor_elements ON documents USING gin(cor_elements);
CREATE INDEX idx_doc_tags ON documents USING gin(tags);
CREATE INDEX idx_doc_applicable_to ON documents USING gin(applicable_to);

-- Revisions
CREATE INDEX idx_revision_document ON document_revisions(document_id);
CREATE INDEX idx_revision_company ON document_revisions(company_id);
CREATE INDEX idx_revision_changed_at ON document_revisions(changed_at DESC);

-- Approvals
CREATE INDEX idx_approval_document ON document_approvals(document_id);
CREATE INDEX idx_approval_company ON document_approvals(company_id);
CREATE INDEX idx_approval_approver ON document_approvals(approver_id) WHERE status = 'pending';
CREATE INDEX idx_approval_status ON document_approvals(status);

-- Reviews
CREATE INDEX idx_review_document ON document_reviews(document_id);
CREATE INDEX idx_review_company ON document_reviews(company_id);
CREATE INDEX idx_review_due_date ON document_reviews(review_due_date) WHERE review_status IN ('scheduled', 'pending', 'overdue');
CREATE INDEX idx_review_assigned ON document_reviews(review_assigned_to) WHERE review_status IN ('pending', 'in_progress');
CREATE INDEX idx_review_status ON document_reviews(review_status);

-- Distributions
CREATE INDEX idx_dist_document ON document_distributions(document_id);
CREATE INDEX idx_dist_company ON document_distributions(company_id);
CREATE INDEX idx_dist_recipient ON document_distributions(distributed_to);
CREATE INDEX idx_dist_unacknowledged ON document_distributions(company_id, acknowledged) WHERE acknowledged = false;

-- Archive
CREATE INDEX idx_archive_company ON document_archive(company_id);
CREATE INDEX idx_archive_control ON document_archive(control_number);
CREATE INDEX idx_archive_document ON document_archive(document_id);
CREATE INDEX idx_archive_destruction ON document_archive(can_be_destroyed_after) WHERE destruction_hold = false;
