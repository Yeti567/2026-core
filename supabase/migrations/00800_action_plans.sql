...
CREATE TABLE IF NOT EXISTS action_phases (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Task Comments/Notes
CREATE TABLE IF NOT EXISTS action_task_notes (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    task_id UUID NOT NULL REFERENCES action_tasks(id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS action_task_dependencies (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    task_id UUID NOT NULL REFERENCES action_tasks(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_action_phases_plan_id ON action_phases(plan_id);
CREATE INDEX IF NOT EXISTS idx_action_tasks_plan_id ON action_tasks(plan_id);
CREATE INDEX IF NOT EXISTS idx_action_tasks_phase_id ON action_tasks(phase_id);
CREATE INDEX IF NOT EXISTS idx_action_tasks_assigned_to ON action_tasks(assigned_to);
CREATE INDEX IF NOT EXISTS idx_action_tasks_status ON action_tasks(status);
CREATE INDEX IF NOT EXISTS idx_action_tasks_due_date ON action_tasks(due_date);
CREATE INDEX IF NOT EXISTS idx_action_tasks_element ON action_tasks(element_number);
CREATE INDEX IF NOT EXISTS idx_action_subtasks_task_id ON action_subtasks(task_id);
CREATE INDEX IF NOT EXISTS idx_action_task_notes_task_id ON action_task_notes(task_id);
...
DROP POLICY IF EXISTS "action_phases_select_policy" ON action_phases;
...
DROP POLICY IF EXISTS "action_phases_insert_policy" ON action_phases;
CREATE POLICY "action_phases_insert_policy" ON action_phases
    FOR INSERT WITH CHECK (
        EXISTS (SELECT 1 FROM action_plans WHERE id = action_phases.plan_id AND (company_id = get_user_company_id() OR is_super_admin()))
    );

DROP POLICY IF EXISTS "action_phases_update_policy" ON action_phases;
CREATE POLICY "action_phases_update_policy" ON action_phases
    FOR UPDATE USING (
        EXISTS (SELECT 1 FROM action_plans WHERE id = action_phases.plan_id AND (company_id = get_user_company_id() OR is_super_admin()))
    );

DROP POLICY IF EXISTS "action_phases_delete_policy" ON action_phases;
...
DROP POLICY IF EXISTS "action_tasks_select_policy" ON action_tasks;
...
DROP POLICY IF EXISTS "action_tasks_insert_policy" ON action_tasks;
CREATE POLICY "action_tasks_insert_policy" ON action_tasks
    FOR INSERT WITH CHECK (
        EXISTS (SELECT 1 FROM action_plans WHERE id = action_tasks.plan_id AND (company_id = get_user_company_id() OR is_super_admin()))
    );

DROP POLICY IF EXISTS "action_tasks_update_policy" ON action_tasks;
CREATE POLICY "action_tasks_update_policy" ON action_tasks
    FOR UPDATE USING (
        EXISTS (SELECT 1 FROM action_plans WHERE id = action_tasks.plan_id AND (company_id = get_user_company_id() OR is_super_admin()))
    );

DROP POLICY IF EXISTS "action_tasks_delete_policy" ON action_tasks;
...
DROP POLICY IF EXISTS "action_subtasks_select_policy" ON action_subtasks;
...
DROP POLICY IF EXISTS "action_subtasks_insert_policy" ON action_subtasks;
...
DROP POLICY IF EXISTS "action_subtasks_update_policy" ON action_subtasks;
...
DROP POLICY IF EXISTS "action_subtasks_delete_policy" ON action_subtasks;
