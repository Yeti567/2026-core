...
CREATE TABLE form_evidence_mappings (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
-- Form Templates
CREATE INDEX idx_form_templates_company ON form_templates(company_id, is_active);
CREATE INDEX idx_form_templates_cor_element ON form_templates(cor_element) WHERE cor_element IS NOT NULL;
CREATE INDEX idx_form_templates_code ON form_templates(form_code);

-- Form Sections
CREATE INDEX idx_form_sections_template ON form_sections(form_template_id, order_index);

-- Form Fields
CREATE INDEX idx_form_fields_section ON form_fields(form_section_id, order_index);
CREATE INDEX idx_form_fields_code ON form_fields(field_code);

-- Form Submissions
CREATE INDEX idx_form_submissions_company ON form_submissions(company_id, submitted_at DESC);
CREATE INDEX idx_form_submissions_template ON form_submissions(form_template_id, status);
CREATE INDEX idx_form_submissions_sync ON form_submissions(company_id, synced) WHERE synced = false;
CREATE INDEX idx_form_submissions_status ON form_submissions(status, submitted_at DESC);
CREATE INDEX idx_form_submissions_submitter ON form_submissions(submitted_by, submitted_at DESC);
CREATE INDEX idx_form_submissions_jobsite ON form_submissions(jobsite_id) WHERE jobsite_id IS NOT NULL;
CREATE INDEX idx_form_submissions_form_data ON form_submissions USING GIN (form_data);

-- Form Evidence Mappings
CREATE INDEX idx_form_evidence_submission ON form_evidence_mappings(form_submission_id);
CREATE INDEX idx_form_evidence_element ON form_evidence_mappings(audit_element);
