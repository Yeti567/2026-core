...
-- Maintenance Records
CREATE INDEX idx_maintenance_records_equipment ON maintenance_records(equipment_id, actual_date DESC);
CREATE INDEX idx_maintenance_records_company ON maintenance_records(company_id, actual_date DESC);
CREATE INDEX idx_maintenance_records_type ON maintenance_records(company_id, maintenance_type);
CREATE INDEX idx_maintenance_records_status ON maintenance_records(company_id, status);
CREATE INDEX idx_maintenance_records_scheduled ON maintenance_records(company_id, scheduled_date) 
    WHERE status = 'scheduled';
CREATE INDEX idx_maintenance_records_followup ON maintenance_records(company_id, followup_required) 
    WHERE followup_required = true;

-- Maintenance Attachments
CREATE INDEX idx_maintenance_attachments_equipment ON maintenance_attachments(equipment_id, uploaded_at DESC);
CREATE INDEX idx_maintenance_attachments_record ON maintenance_attachments(maintenance_record_id);
CREATE INDEX idx_maintenance_attachments_company ON maintenance_attachments(company_id, attachment_type);
CREATE INDEX idx_maintenance_attachments_type ON maintenance_attachments(attachment_type, uploaded_at DESC);
CREATE INDEX idx_maintenance_attachments_tags ON maintenance_attachments USING gin(tags);

-- Maintenance Schedules
CREATE INDEX idx_maintenance_schedules_equipment ON maintenance_schedules(equipment_id, is_active);
CREATE INDEX idx_maintenance_schedules_due ON maintenance_schedules(company_id, next_due_date) 
    WHERE is_active = true;
CREATE INDEX idx_maintenance_schedules_due_hours ON maintenance_schedules(company_id, next_due_hours) 
    WHERE is_active = true AND next_due_hours IS NOT NULL;
CREATE INDEX idx_maintenance_schedules_mandatory ON maintenance_schedules(company_id, is_mandatory) 
    WHERE is_mandatory = true AND is_active = true;

-- Work Orders
CREATE INDEX idx_work_orders_status ON work_orders(company_id, status, scheduled_start);
CREATE INDEX idx_work_orders_equipment ON work_orders(equipment_id, status);
CREATE INDEX idx_work_orders_priority ON work_orders(company_id, priority) 
...
CREATE INDEX idx_downtime_equipment ON equipment_downtime_log(equipment_id, downtime_start DESC);
CREATE INDEX idx_downtime_company ON equipment_downtime_log(company_id, downtime_start DESC);
CREATE INDEX idx_downtime_active ON equipment_downtime_log(equipment_id) 
    WHERE downtime_end IS NULL;
CREATE INDEX idx_downtime_reason ON equipment_downtime_log(company_id, reason);
